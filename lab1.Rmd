...
var_to_group <- 
threshold <- 
is_delay <- ifelse(df$arr_delay>threshold,1,0)
# groupby
value_count <- as.data.frame(group_by(df,var_to_group) %>% summarise(is_delay = sum(is_delay, na.rm = TRUE)))
# total computation
  total <- data.frame(table(df$var_to_group))
  names(total) <- c(var_to_group, 'total')
  comp_df <- left_join(value_count,total, by=var_to_group)
  comp_df$perc <- comp_df[var_to_group]/comp_df$total
  return(comp_df$perc)
}
```

```{r}
flights_planes <- left_join(flights,planes,"tailnum")
lin_reg <- lm(dep_delay~carrier+origin+distance+type+manufacturer+model+engines, data = flights_planes)
#summary(lin_reg)
```

```{r}
df <- as.data.frame(group_by(flights,carrier) %>% summarise(dep_delay = mean(dep_delay, na.rm = TRUE)))
names(df) <- c("carrier","dep_delay")
head(df)
#ggplot(data=df) +
 # geom_bar(aes(x=carrier,y=dep_delay),stat = "identity")
```
...
stam1 <- data %>%
  ...
  summarise(average_delay = mean(dep_delay, na.rm = 1) ,std = sd(dep_delay, na.rm = 1))
#stam1$std <- stam1$std/sqrt(table(data$day_num_of_week))

ggplot(data=stam1, aes(x=day_num_of_week,y=average_delay)) +
  geom_point(aes(x=day_num_of_week,y=average_delay)) +
  geom_errorbar(aes(ymin=average_delay-std,ymax=average_delay+std), width = 0.1) +
...
```{r}
stam <- data %>%
  group_by(day_num_of_week, hour) %>%
  summarise(average_delay = mean(dep_delay, na.rm = 1) ,std = sd(dep_delay, na.rm = 1))
stam$daytime <- cut(stam$hour, c(0,4,11,18,23),labels=c("evening (18h-01h)","morning (05h-11h)","afternoon (11h-18h)","evening (18h-01h)"),right = TRUE)


ggplot(data=stam) +
  ggtitle("Departures Delays per Day of Week") +
  labs(x="Day", y="Avg Delay (minutes)") +
  # per daytime
  geom_point(aes(x=day_num_of_week,y=average_delay,colour=daytime), show.legend = TRUE) +
  # daily
  geom_point(data = stam1,aes(x=day_num_of_week,y=average_delay, fill="daily avg"), size=2.5) +
  labs(fill="") +
  geom_line(data = stam1,aes(x=day_num_of_week,y=average_delay)) +
  geom_text(data=stam1, aes(x=day_num_of_week,y=average_delay+2, label=round(average_delay,digits = 2)),fontface='bold',size=4) +
